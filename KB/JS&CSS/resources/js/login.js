...
logbtn.addEventListener('submit', function(e) {
    ...
    if(id == 'test') {
       ...
       Swal.fire('입력 오류', '아이디나 비밀번호를 입력해주세요.', 'error');
    } else {
       Swal.fire('아이디 오류', '아이디를 확인해주세요.', 'error');
    }
});
...
uLogbtn.addEventListener('submit', function(e) {
    ...
    if(uId == 'test') {
        if(uPw == '1234') {
           ...
           Swal.fire('주문번호 오류', '주문번호를 확인해주세요.', 'error');
        } 
    } else if((uId == '') || (uPw == '')) {
       Swal.fire('입력 오류', '주문자명이나 주문번호를 입력해주세요.', 'error');
    } else {
       Swal.fire('주문자명 오류', '주문자명를 확인해주세요.', 'error');
    }
});
